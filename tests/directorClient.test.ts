...
const okDirective = {
  ...
  mutation: 'FOG', buff: 'NONE', deny: 'NONE', taunt: '벽을 좋아하는군.', intent: '벽 차단',
};
...
describe('requestDirective', () => {
  it('정상 응답이면 LLM 디렉티브 반환', async () => {
    ...
    const r = await requestDirective(fakeLog, 3, 'NONE', 'NONE', 'NONE');
  ...
  it('4초 초과면 폴백', async () => {
    ...
    const p = requestDirective(fakeLog, 3, 'NONE', 'NONE', 'NONE');
