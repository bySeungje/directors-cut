...
/** 웨이브별 스폰 예산: **10/14/18/22/28/34/40**.
 *
 *  2026-08-09 재설계. 맵/벽이 생긴 뒤에는 동시 개체 수가 아니라 순찰 밀도와 시야 겹침이 난이도를
 *  만든다. 예전 총쏘기 곡선(19/26/33/40/50/66/88)을 유지하면 경비가 벽에 막혀도 화면이 물량전으로
 *  읽힌다. 후반도 40점 안쪽으로 묶어, 플레이어가 길을 읽고 피하거나 필요할 때만 교란하는 게임으로 둔다. */
export function budgetFor(wave: number): number {
  return 6 + wave * 4 + Math.max(0, wave - 4) * 2;
}
