...
/** 웨이브별 스폰 예산: **12/16/20/24/32/44/60**.
 *
 *  2026-08-10 하향(승제 판정: "몬스터를 줄이고 진짜 움직임을 예측해서 오브젝트가 날 적중하려 한다던지").
 *  물량이 주인공 자리를 뺏고 있었다 — 화면이 적으로 꽉 차면 예고·집행자·예측 타격이 전부 묻힌다.
 *  적은 무대이지 주인공이 아니다. 구 곡선(19/26/33/40/50/66/88) 대비 약 35% 감축.
 *
 *  구 주석은 아래에 남긴다 — 왜 한 번 올렸었는지가 다음 세션이 틀릴 만한 정보다.
 *
 *  (구) 웨이브별 스폰 예산: **19/26/33/40/50/66/88**.
 *
 *  2026-08-08 재설계. 구 곡선(12/16/20/24/34/56/90)은 초중반이 너무 낮아 6웨이브까지 아무 위협이
 *  없었다(승제 실측: 7웨이브 2회 클리어에 피격 1회). 다만 **곡선만으로는 고쳐지지 않는다** —
 *  추격형이 플레이어보다 2.4배 느리고 shooter가 현재 위치를 쏴서, 움직이면 물량이 늘어도 그냥
 *  지나쳐 갔다. 위협 모델(`entities.ts`의 이속·탄속·선행 조준)을 먼저 고친 뒤 이 곡선을 얹었다.
 *
 *  초반부터 올리되 최종은 92로 묶는다 — 실측상 동시 100기 이상은 이동해도 위험한 즉사 구간이고,
 *  심사자가 못 깨면 엔드 리포트라는 두 번째 AI 장면을 아예 못 본다. */
export function budgetFor(wave: number): number {
  return 8 + wave * 4 + Math.max(0, wave - 4) ** 2 * 2;
}
